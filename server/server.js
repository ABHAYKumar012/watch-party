...
function getLiveVideoState(room) {
  ...
  if (v.playState === 'playing') {
    currentTime += (Date.now() - v.lastUpdate) / 1000;
  }
  return {
    videoId: v.videoId,
...
function touchVideoState(room, patch) {
  ...
  room.video = {
    videoId: patch.videoId !== undefined ? patch.videoId : live.videoId,
    playState: patch.playState !== undefined ? patch.playState : live.playState,
    currentTime: patch.currentTime !== undefined ? patch.currentTime : live.currentTime,
    lastUpdate: Date.now()
  };
}
...
function broadcastSyncState(room) {
  const live = getLiveVideoState(room);
  io.to(room.id).emit('sync_state', live);
}
...
setInterval(() => {
  for (const room of rooms.values()) {
    if (room.video.playState === 'playing' && room.participants.size > 0) {
...
io.on('connection', (socket) => {
  ...
  socket.on('create_room', ({ username }, cb) => {
    ...
    const room = {
      ...
      video: { videoId: null, playState: 'paused', currentTime: 0, lastUpdate: Date.now() },
      chat: []
    };
    room.participants.set(userId, {
      id: userId,
      username: (username || 'Host').trim().slice(0, 30),
      role: ROLES.HOST,
      socketId: socket.id,
      joinedAt: Date.now()
    });
    ...
    if (typeof cb === 'function') {
      cb({ ok: true, roomId, userId, role: ROLES.HOST, ...roomSnapshot(room) });
    }
  });
  ...
  socket.on('join_room', ({ roomId, username }, cb) => {
    ...
    if (!room) {
      if (typeof cb === 'function') cb({ ok: false, error: 'Room not found. Check the code and try again.' });
      return;
    }
    ...
    const participant = {
      id: userId,
      username: (username || 'Guest').trim().slice(0, 30) || 'Guest',
    ...
    if (typeof cb === 'function') {
      cb({ ok: true, roomId: room.id, userId, role: participant.role, ...roomSnapshot(room) });
    }
    ...
    // Send the joiner an immediate, accurate sync so their player starts in the right spot.
    socket.emit('sync_state', getLiveVideoState(room));
  });
  ...
  socket.on('play', ({ currentTime } = {}) => {
    const { room, participant } = context(socket);
    if (!room || !participant) return;
    if (!canControlPlayback(participant.role)) {
      return socket.emit('error_message', { error: 'Only the host or a moderator can control playback.' });
    }
    touchVideoState(room, { playState: 'playing', currentTime });
    broadcastSyncState(room);
  });
  ...
  socket.on('pause', ({ currentTime } = {}) => {
    ...
    touchVideoState(room, { playState: 'paused', currentTime });
    broadcastSyncState(room);
  });
  ...
  socket.on('seek', ({ time } = {}) => {
    ...
    if (!room || !participant) return;
    if (!canControlPlayback(participant.role)) {
      return socket.emit('error_message', { error: 'Only the host or a moderator can seek.' });
    }
    touchVideoState(room, { currentTime: time });
    broadcastSyncState(room);
  });
  ...
  socket.on('change_video', ({ videoId } = {}) => {
    ...
    if (!room || !participant) return;
    if (!canControlPlayback(participant.role)) {
      return socket.emit('error_message', { error: 'Only the host or a moderator can change the video.' });
    }
    if (!videoId) return;
    touchVideoState(room, { videoId, playState: 'playing', currentTime: 0 });
    broadcastSyncState(room);
  });
  ...
  socket.on('assign_role', ({ userId, role } = {}) => {
    ...
    if (!isHost(participant.role)) {
      return socket.emit('error_message', { error: 'Only the host can assign roles.' });
    }
    if (!Object.values(ROLES).includes(role) || role === ROLES.HOST) {
      return socket.emit('error_message', { error: 'Invalid role.' });
    }
    const target = room.participants.get(userId);
    if (!target) return;
    if (target.id === room.hostId) return; // can't demote yourself this way; use transfer_host
  ...
  socket.on('remove_participant', ({ userId } = {}) => {
    ...
    if (!room || !participant) return;
    if (!isHost(participant.role)) {
      return socket.emit('error_message', { error: 'Only the host can remove participants.' });
    }
    const target = room.participants.get(userId);
  ...
  socket.on('transfer_host', ({ userId } = {}) => {
    ...
    if (!room || !participant) return;
    if (!isHost(participant.role)) {
      return socket.emit('error_message', { error: 'Only the host can transfer host.' });
    }
    const target = room.participants.get(userId);
    if (!target) return;
  ...
  function context(sock) {
    ...
    if (!room) return {};
    const participant = room.participants.get(sock.data.userId);
  ...
  function handleLeave(sock) {
    ...
    const leaving = room.participants.get(userId);
    if (!leaving) return;
